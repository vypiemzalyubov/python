...
`in` - проверить, есть ли элемент в списке. Может работать с логическим оператором `not`.
```python
numbers = [2, 4, 6, 8, 10]
...
:exclamation: Индексация, срезы, конкатенация, умножение на число,встроенные функции `sum()`, `min()`, `max()`, оператор `in`, длина `len()` - работают **по аналогии** со строками.

Отличие списков от строк в том, что **строки - неизменяемые объекты**, а **списки - изменяемые**. Изменять отдельные символы строк нельзя, однако можно изменять отдельные элементы списков. Для этого используется индексатор и оператор присваивания.
---
