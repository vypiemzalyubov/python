...
# 186. Напишите программу, которая при помощи метода Монте-Карло вычисляет площадь фигуры, задаваемой с помощью системы неравенств:
#      ⎧ −2≤x≤2
#      ⎨ −2≤y≤2
#      ⎩ x**3+y**4+2≥0
#        3x+y**2≤2
...
for _ in range(n):
    ...
    if 3 * x + y ** 2 <= 2 and x ** 3 + y ** 4 + 2 >= 0:
        k += 1

print((k/n)*s0)

# 187. Напишите программу, которая при помощи метода Монте-Карло определяет приближённое значение числа π.
