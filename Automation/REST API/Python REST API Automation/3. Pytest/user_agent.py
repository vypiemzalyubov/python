...
class TestUserAgent:
    ...
    @pytest.mark.parametrize("expected_result", expected_results)
    def test_check_user_agent(self, user_agent, expected_result):
        ...
        if user_agent == "Mozilla/5.0 (Linux; U; Android 4.0.2; en-us; Galaxy Nexus Build/ICL53F) AppleWebKit/534.30 (KHTML, like Gecko) Version/4.0 Mobile Safari/534.30":
            ...
            print('AAAAAAAAAAAAAAAA', expected_result[0]["platform"])
            assert response1.status_code == 200, "Wrong response status code"
            assert response1.json()["platform"] == expected_result[0]["platform"] and response1.json()["browser"] == 'No' and response1.json()["device"] == 'Android', f"Wrong answer to request with user-agent {response1.request.headers}"
